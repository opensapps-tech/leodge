...
def tpl_package_json(app_name, bundle_id, pkg_mgr):
    lock = "npm install" if pkg_mgr == "npm" else "yarn install"
    return f"""\
{{
  "name": "{app_name.lower()}",
  "version": "1.0.0",
...
// Must use plugins {} block (not legacy apply plugin:) for plugins resolved
// via includeBuild in pluginManagement — which is how RN ships its Gradle plugin.
plugins {
...
# Configuration cache is DISABLED — React Native autolinking spawns an external
# process (npx @react-native-community/cli config) during configuration phase
# which Gradle 8's config cache explicitly forbids.
...
# JS bundling is handled explicitly in CI (see .github/workflows/build.yml)
# via `react-native bundle` before assembleDebug runs.
# bundleInDebug=true is NOT used — it is unreliable in RN 0.75's Gradle plugin.
...
def tpl_github_actions(app_name, bundle_id, pkg_mgr, gh_repo):
    install_cmd = "npm install" if pkg_mgr == "npm" else "yarn install"
...
jobs:
  build-android:
    ...
    steps:
      ...
      # ── Node 20 LTS ─────────────────────────────────────────────
      # Note: no 'cache:' here — setup-node's built-in caching requires a
      # lock file to exist at checkout time, which won't be true on the first
      # push of a freshly generated repo. We cache node_modules explicitly
      # with actions/cache after install instead.
      - name: Set up Node 20
      ...
      # Run install BEFORE caching so the lock file exists for hashing.
      ...
      # ── Explicitly bundle JS into assets ────────────────────────
      # This is the only reliable way to get JS into a CI-built APK.
      # bundleInDebug=true in gradle.properties is NOT used — it is
      # inconsistently honoured by the RN 0.75 Gradle plugin.
      # Gradle finds the pre-built bundle in assets/ and skips its own
      # (broken) bundling step automatically.
...
def print_next_steps(base, cfg):
    ...
    gh_repo   = cfg["gh_repo"]
    pkg_mgr   = cfg["pkg_mgr"]
    install   = "npm install" if pkg_mgr == "npm" else "yarn install"
